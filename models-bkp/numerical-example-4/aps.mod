...
# param RealocacaoEquipe, symbolic, default "3-RealocacaoEquipe.txt";
# param ContratacaoEquipe, symbolic, default "4-ContratacaoEquipe.txt";
# param Balanceamento, symbolic, default "5-Balanceamento.txt";
# param Fluxo, symbolic, default "6-Fluxo.txt";
# param Uso, symbolic, default "7-Uso.txt";
...
#       sum{i in I, j1 in L1}VC1[j1]*u0_1[i,j1] 
#     + sum{j1 in L1, j2 in L2}VC2[j2]*u1_2[j1,j2]  
#     + sum{j2 in L2, j3 in L3}VC3[j3]*u2_3[j2,j3] >> Financeiro;
# printf: "========================================\n";
# printf: "Total Cost:\t$%10.2f\n", Total_Costs >> Financeiro;
# printf: "Total APS Cost:\t$%10.2f\n", Total_Costs_APS >> Financeiro;
# printf: "Budget Limit:\t$%10.2f\n", BUDGET >> Financeiro;
# printf: "Budget Usage:\t%.2f%%\n", (Total_Costs_APS/BUDGET)*100 >> Financeiro;
# printf: "========================================\n";
...
# sum{j1 in CL[1] inter L1}y1[j1],
# MaxNewPHC, 
# if MaxNewPHC > 0 then ((sum{j1 in CL[1] inter L1}y1[j1])/(MaxNewPHC))*100 else 0 >> NovasUnidades;
# printf: "SHC:\t%d\t%d\t%.2f%%\n", 
# sum{j2 in CL[2] inter L2}y2[j2],
# MaxNewSHC, 
# if MaxNewSHC > 0 then ((sum{j2 in CL[2] inter L2}y2[j2])/(MaxNewSHC))*100 else 0 >> NovasUnidades;
# printf: "THC :\t%d\t%d\t%.2f%%\n", 
# sum{j3 in CL[3] inter L3}y3[j3],
# MaxNewTHC, 
# if MaxNewTHC > 0 then ((sum{j3 in CL[3] inter L3}y3[j3])/(MaxNewTHC))*100 else 0 >> NovasUnidades;
# printf: "========================================\n";
...
# for{e1 in E[1], from in EL[1] inter L1, to in L1: from != to and transfer1[e1,from,to] > 0.01}{
#     printf: "[%-4s]\t[%-4s]\t%-4s\t%.2f\t%.0f\n", 
#         from, to, e1, transfer1[e1,from,to], DL1[from,to] >> RealocacaoEquipe;
# }
...
# for{e2 in E[2], from in EL[2] inter L2, to in L2: from != to and transfer2[e2,from,to] > 0.01}{
#     printf: "[%-4s]\t[%-4s]\t%-4s\t%.2f\t%.0f\n", 
#         from, to, e2, transfer2[e2,from,to], DL2[from,to] >> RealocacaoEquipe;
# }
...
# for{e3 in E[3], from in EL[3] inter L3, to in L3: from != to and transfer3[e3,from,to] > 0.01}{
#     printf: "[%-4s]\t[%-4s]\t%-4s\t%.2f\t%.0f\n", 
#         from, to, e3, transfer3[e3,from,to], DL3[from,to] >> RealocacaoEquipe;
# }
...
#         if j1 in CL[1] then "*" else " ",
#         e1, 
#         newhire1[e1,j1] >> ContratacaoEquipe;
# }
...
#         if j2 in CL[2] then "*" else " ",
#         e2, 
#         newhire2[e2,j2] >> ContratacaoEquipe;
# }
...
#         if j3 in CL[3] then "*" else " ",
#         e3, 
#         newhire3[e3,j3] >> ContratacaoEquipe;
# }
...
#         + sum{j2 in L2}u2_1[j2,j1]
#         + sum{j3 in L3}u3_1[j3,j1],
#         sum{i in I}(u0_1[i,j1]+ ut1[i,j1]), sum{j2 in L2}u2_1[j2,j1], sum{j3 in L3}u3_1[j3,j1] >> Fluxo; 
#     # Reg
#     for{i in I: u1_0[j1,i] > 0}{
#     printf"\tL[%-4s]\t%d\n", i, u1_0[j1,i] >> Fluxo;} # O1_0[j1]*
#     # SHC
#     for{j2 in L2: u1_2[j1,j2] > 0}{
#     printf"\tL[%-4s]\t%d\n", j2, u1_2[j1,j2] >> Fluxo;} # O1_2[j1]*
#     # THC
#     for{j3 in L3: u1_3[j1,j3] > 0}{
#     printf"\tL[%-4s]\t%d\n", j3, u1_3[j1,j3] >> Fluxo;} # O1_3[j1]*
# }
...
#         + sum{j1 in L1}u1_2[j1,j2] 
#         + sum{j3 in L3}u3_2[j3,j2], 
#         sum{i in I}(u0_2[i,j2] + ut2[i,j2]), sum{j1 in L1}u1_2[j1,j2], sum{j3 in L3}u3_2[j3,j2] >> Fluxo;
#     # Reg
#     for{i in I: u2_0[j2,i] > 0}{
#     printf"\tL[%-4s]\t%d\n", i, u2_0[j2,i] >> Fluxo;} 
#     # PHC
...
#         + sum{j1 in L1}u1_3[j1,j3] 
#         + sum{j2 in L2}u2_3[j2,j3], 
#         sum{i in I}(u0_3[i,j3] + ut3[i,j3]), sum{j1 in L1}u1_3[j1,j3], sum{j2 in L2}u2_3[j2,j3] >> Fluxo;
#     # Reg
#     for{i in I: u3_0[j3,i] > 0}{
#     printf"\tL[%-4s]\t%d\n", i, u3_0[j3,i] >> Fluxo;} 
#     # PHC
...
# C1[j1], 
# (sum{i in I}(u0_1[i,j1]+ ut1[i,j1]) + sum{j2 in L2}u2_1[j2,j1] + sum{j3 in L3}u3_1[j3,j1]),
# if C1[j1] > 0 then ((sum{i in I}(u0_1[i,j1]+ ut1[i,j1]) + sum{j2 in L2}u2_1[j2,j1] + sum{j3 in L3}u3_1[j3,j1])/(C1[j1]))*100 else 0 >> Uso;
...
# C2[j2], 
# (sum{i in I}(u0_2[i,j2] + ut2[i,j2]) + sum{j1 in L1}u1_2[j1,j2] + sum{j3 in L3}u3_2[j3,j2]),
# if C2[j2] > 0 then ((sum{i in I}(u0_2[i,j2] + ut2[i,j2]) + sum{j1 in L1}u1_2[j1,j2] + sum{j3 in L3}u3_2[j3,j2])/(C2[j2]))*100 else 0 >> Uso;
...
# C3[j3], 
# (sum{i in I}(u0_3[i,j3] + ut3[i,j3]) + sum{j1 in L1}u1_3[j1,j3] + sum{j2 in L2}u2_3[j2,j3]),
# if C3[j3] > 0 then ((sum{i in I}(u0_3[i,j3] + ut3[i,j3]) + sum{j1 in L1}u1_3[j1,j3] + sum{j2 in L2}u2_3[j2,j3])/(C3[j3]))*100 else 0 >> Uso;
